...
abstract public class UIComponent extends Region {
    ...
    /**
     * Sets a CSS-file from /view/css/ on the UI component.
     *
     * @param fileName The name of the file (e.g. "application.css")
     */
    public void loadCSS(final String fileName) {
        UIComponent.logger.info("Loaded CSS file /view/css/%s into UIComponent %s.", fileName, this.getName());
        // apply the stylesheet
        Platform.runLater(() -> this.getScene().getStylesheets()
                .addAll(this.getClass().getResource("/view/css/" + fileName).toExternalForm()));
    }
    ...
    public boolean loadFXML(final String fileName) {
        try {
            // load the file
            final FXMLLoader fxmlLoader = new FXMLLoader(this.getClass().getResource("/view/components/" + fileName));
