 ...
 * The Class Producer.
 * 
 * A Producer generates packets of the type (or subtype of) {@link PipelinePacket}. It does this
 * by running single or scheduled tasks.
 *
 * @author Rimon Oz
 * @param <T> A PipelinePacket this Producer can work with.
 */
abstract public class Producer<T extends PipelinePacket> extends Service<T> {
     ...
     * @param producerType The type of the new Producer
     */
    public Producer(String name, ProducerType producerType) {
        super(name);
        // set attributes
     ...
     */
    public ListenableFuture<T> produceOnce(String taskName) {
     ...
     */
    public ListenableFuture<T> produceSchedule(String taskName, long taskInterval) {
        // add the task to the scheduled worker pool
        final ListenableScheduledFuture<T> producerFuture = Services.scheduleTask(taskName, this.getRunnable(), taskInterval);
        Futures.addCallback(producerFuture, new FutureCallback<T>() {
            /**
             * Task failed to execute or threw an error!
             */            
    ...
    /**
     * Registers a {@link pipeline.Pipeline} with the Producer.
     *
     * @param pipeline The pipeline to be registered.
     * @return         true, if successful.
     */
    public boolean registerPipeline(Pipeline<T> pipeline) {
     ...
     */
    public void setCallable(Callable<T> workFunction) {
     ...
     *
     * @param producerType The new type of the producer.
     */
