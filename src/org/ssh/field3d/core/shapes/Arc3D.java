...
public class Arc3D {
    ...
    private final TriangleMesh _triangleMesh;
    
    public Arc3D(final double startAngle,
            final double endAngle,
            final double radius,
            final double thickness,
            final int numDivisions) {
            
        this._triangleMesh = new TriangleMesh(VertexFormat.POINT_TEXCOORD);
        
        // Calculate vertices
        final float[] vertices = this.calculateVertices(startAngle, endAngle, radius, thickness, numDivisions);
        this._triangleMesh.getPoints().addAll(vertices);
        
        // Calculate texture coords
        final float[] texCoords = { 1, 1, // idx t0
                1, 0, // idx t1
                0, 1, // idx t2
                0, 0 // idx t3
        };
        
        this._triangleMesh.getTexCoords().addAll(texCoords);
        
        final int faces[] = this.calculateFaces(numDivisions);
        this._triangleMesh.getFaces().addAll(faces);
    }
    ...
    private float[] calculateVertices(final double startAngle,
        ...
        final double deltaAngle = totalAngle / numDivisions;
        double curAngle = startAngle;
        
        final double deltaAngleRad = (deltaAngle * Math.PI) / 180.0;
        double curAngleRad = (curAngle * Math.PI) / 180.0;
        
        final int amountOfVertices = (numDivisions + 1) * 2;
        ...
        for (int i = 0; i < (numDivisions + 1); i++) {
            ...
            final double b1y = Math.sin(curAngleRad) * (radius - halfOfThickness);
            final double b11x = Math.cos(curAngleRad) * (radius + halfOfThickness);
            final double b11y = Math.sin(curAngleRad) * (radius + halfOfThickness);
            
            final Vector3f vectorB1 = new Vector3f((float) b1x, (float) b1y, 0.0f);
            final Vector3f vectorB11 = new Vector3f((float) b11x, (float) b11y, 0.0f);
    ...
    public MeshView MeshView() {
        return new MeshView(this._triangleMesh);
    }
