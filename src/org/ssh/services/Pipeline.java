 ...
 * The Class Pipeline.
 * 
 * A Pipeline processes data using {@link org.ssh.services.pipeline.Producer}, {@link org.ssh.services.pipeline.Coupler}, and {@link org.ssh.services.pipeline.Consumer}.
 *
 * @author Rimon Oz
...
public abstract class Pipeline<T extends PipelinePacket> {
    ...
    public Pipeline<T> addPacket(T pipelinePacket) {
        ...
        Pipeline.logger.info("Packet of type %s added to org.ssh.services.pipeline %s ...", pipelinePacket.getClass().toString(), this.getName());
    ...
    public boolean processPacket() {
        ...
        Pipeline.logger.info("Packet pushed through all couplers, now mapping to consumers on org.ssh.services.pipeline %s", this.getName());
    ...
    public <P extends PipelinePacket, C extends Consumer<P>> boolean registerConsumer(C consumer) {
        Pipeline.logger.info("Consumer named %s registered to org.ssh.services.pipeline %s.", consumer.getName(), this.getName());
    ...
    public <P extends PipelinePacket, C extends Coupler<P>> boolean registerCoupler(C coupler) {
        Pipeline.logger.info("Coupler named %s registered to org.ssh.services.pipeline %s.", coupler.getName(), this.getName());
        return this.couplers.put((Coupler<T>) coupler, Priority.MEDIUM) != null;
    }

    /**
     * Registers a {@link org.ssh.services.pipeline.Coupler} with the Pipeline with the given Priority.
     *
    ...
    public boolean registerCoupler(Priority priority, Coupler<T> coupler) {
        Pipeline.logger.info("Consumer named %s registered to org.ssh.services.pipeline %s with priority %s.", coupler.getName(), this.getName(), priority.toString());
    ...
    /**
     * Registers a list of {@link org.ssh.services.pipeline.Coupler} with the Pipeline with the given Priority.
     *
