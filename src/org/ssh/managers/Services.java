...
public final class Services extends Manager<Service<? extends PipelinePacket>> {
    ...
    /**
     * Adds a pipeline that can be used by a
     * {@link Producer}, {@link Coupler}, or
     * {@link Consumer}.
     * @param <P>
    ...
    public static <P extends PipelinePacket> boolean addPipeline(final Pipeline<P> pipeline) {
        Services.LOG.info("Adding pipeline<%s>: %s",
            pipeline.getType(),
    ...
    public static void addServices(final Service<? extends PipelinePacket>... services) {
        Stream.of(services)
            .parallel()
            .forEach(service -> Services.controller.add(service));
    }
    
    /**
     * Gets a List of available {@link Consumer} compatible with the given
     * {@link Pipeline}.
     *
     * @param <P>
     *            The generic type of {@link PipelinePacket} handled
     *            by the Consumer.
     * @param <C>
     ...
     */
    public static <P extends PipelinePacket, C extends Consumer<P>> List<C> getCompatibleConsumers(final Pipeline<P> pipeline) {
    ...
    /**
     * Gets a List of available {@link Coupler} compatible with the given
     * {@link Pipeline}.
     *
     * @param <P>
     *            The generic type of {@link PipelinePacket} handled
     ...
     */
    public static <P extends PipelinePacket, C extends Coupler<P>> List<C> getCompatibleCouplers(final Pipeline<P> pipeline) {
    ...
    /**
     * Gets a List of available {@link Producer} compatible with the given
     * {@link Pipeline}.
     *
     * @param <P>
     *            The generic type of {@link PipelinePacket} handled
     ...
     */
    public static <P extends PipelinePacket, C extends Producer<P>> List<C> getCompatibleProducers(final Pipeline<P> pipeline) {
     ...
     *
     * @param <P>            The generic type Consumer compatible with the Pipeline.
     ...
     * @see org.ssh.managers.controllers.ServicesController#submitTask(Callable)
     */
    public static ListenableFuture<?> submitTask(final String taskName,
            final Runnable task) {
        Services.LOG.info("Submitting task named %s ...", taskName);
        return Services.controller.submitTask(task);
    }
