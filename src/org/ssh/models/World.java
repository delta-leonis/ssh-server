...
public class World {
	...
	public Robot getRobot(int id, Color teamColor) {
		return getRobots(teamColor).stream().filter(robot -> robot.getRobotId() == id).findFirst()
				.get();
	}
	 ...
	 * @return a team
	 */
	public Team getTeam(Direction direction) {
		return teams.stream().filter(team -> team.getDirection().equals(direction)).findFirst().get();
	}
}