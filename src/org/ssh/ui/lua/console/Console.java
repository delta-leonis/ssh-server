...
public class Console extends UIComponent {
    ...
    private void addKeyListeners() {
        ...
        EventHandlerHelper.install(this.consoleArea.onKeyPressedProperty(),
                EventHandlerHelper.on(EventPattern.keyPressed(KeyCode.ENTER)).act(event -> this.handleEnter(event))
                        .create());
        EventHandlerHelper.install(this.consoleArea.onKeyPressedProperty(),
                EventHandlerHelper.on(EventPattern.keyPressed(KeyCode.TAB)).act(event -> this.handleTab(event))
                        .create());
    }
    ...
    private List<String> getClasses() {
        if (this.functionClasses == null) return null;
        // Turn everything into a stream
        final List<String> strings = this.functionClasses.stream().map(o ->
    ...
    private List<String> getFunctions() {
        if (this.functionClasses == null) return null;
        // Turn into stream
        final ArrayList<String> strings = (ArrayList<String>) this.functionClasses.stream()
     ...
     */
    private void handleTab(final KeyEvent event) {
