	  ...
	  class RemindTask extends TimerTask {
	    public void run() {
	    	if(shouldBeRunning()){
				...
				ComInterface.getInstance(RobotCom.class).send(1,  World.getInstance().getGUI().getSelectedRobotId(), 0, 0, 0, 0,  Integer.valueOf(textField.getText()),0, false);
				wisselTimer = new Timer();
				wisselTimer.schedule(new RemindTask(),  1000);
	    	}else{
	    		System.out.println("terminated");
	    		ComInterface.getInstance(RobotCom.class).send(1,  World.getInstance().getGUI().getSelectedRobotId(), 0, 0, 0, 0, 0,0, false);
	    	}
