	...
	public void updateAttacker(RobotExecuter executer) {
		...
		if (freeShot != null) {
			if (	// Check whether the angle between robot and ball is lines up with the angle between the freeshot and the ball
					Math.abs(Math.abs(freeShot.getAngle(ballPosition))-Math.abs(ballPosition.getAngle(executer.getRobot().getPosition()))) < 3000 / executer.getRobot().getPosition().getDeltaDistance(ballPosition)
					...
					&& Math.abs(Math.abs(executer.getRobot().getOrientation()) - Math.abs(ballPosition.getAngle(freeShot))) < 2500 / executer.getRobot().getPosition().getDeltaDistance(ballPosition)) {
				chipKick = -60;
			}
//			if (	// Check whether the angle between robot and ball is lines up with the angle between the freeshot and the ball
